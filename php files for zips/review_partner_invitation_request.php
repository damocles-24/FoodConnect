...
try {
    ...
    error_log("review_partner_invitation_request.php error: " . $error->getMessage());
    $status = str_contains(strtolower($error->getMessage()), "already") ? 409 : 500;
    respond_json(["success" => false, "message" => $error->getMessage()], $status);
}

$safeName = escape_html($fullName);
$safeRestaurant = escape_html($restaurant);
if ($decision === "approve") {
    $registrationLink = "http://localhost/FoodConnect/frontend/html/partner_apply.html";
