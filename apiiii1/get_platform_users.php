...
if (!$adminStmt) {
    error_log("get_platform_users.php admin prepare error: " . $conn->error);
    respond_json([
        "success" => false,
...
if (!$adminStmt->execute()) {
    ...
    error_log("get_platform_users.php admin execute error: " . $error);
    respond_json([
        "success" => false,
        "message" => "Unable to verify administrator access."
    ], 500);
...
if (!$usersStmt) {
    error_log("get_platform_users.php users prepare error: " . $conn->error);
    respond_json([
        "success" => false,
        "message" => "Unable to load platform users."
    ], 500);
}
...
if (
    ...
    if (
        ...
        error_log("get_platform_users.php filter bind error: " . $error);
        respond_json([
            "success" => false,
            "message" => "Unable to load platform users."
        ], 500);
...
if (!$usersStmt->execute()) {
    ...
    error_log("get_platform_users.php users execute error: " . $error);
    respond_json([
        "success" => false,
        "message" => "Unable to load platform users."
    ], 500);
...
if (!$summaryResult) {
    error_log("get_platform_users.php summary error: " . $conn->error);
    respond_json([
        "success" => false,
        "message" => "Unable to load platform user summary."
    ], 500);
}
